...
public:
    ...
    shared_ptr<vector<UnifierType>> SolveGoals(HtnGoalResolver *resolver, int memoryBudget = 1000000, int64_t *highestMemoryUsedReturn = nullptr, int *furthestFailureIndex = nullptr, std::vector<std::shared_ptr<HtnTerm>> *farthestFailureContext = nullptr, int64_t *resolutionStepCountReturn = nullptr)
    {
        return resolver->ResolveAll(m_termFactory, m_state, m_goals, 0, memoryBudget, highestMemoryUsedReturn, furthestFailureIndex, farthestFailureContext, resolutionStepCountReturn);
    }
